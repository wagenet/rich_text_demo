...
},_action:function(k){var g=this.$(".sc-button-label")[0];var a=this.get("layer").offsetWidth;
var h=g.scrollWidth;var i=this.get("lastMenuWidth");if(h){var o=g.offsetWidth;if(h&&o){a=a+h-o
}}if(!i||(a>i)){i=a}var m=this.get("itemList");var s,d;for(var l=0;l<m.length;++l){var r=m.objectAt(l);
...
c.innerHTML=r.title;document.body.appendChild(c);s=c.offsetWidth;if(!d||(s>d)){d=s
}document.body.removeChild(c)}i=(d>i)?d:i;this.set("lastMenuWidth",i);var p=this.get("currentSelItem");
var n=this.get("itemList");var q=this.get("controlSize");var e=this.get("menuPaneHeightPadding");
var f=this.get("customView");var j=f?f:SC.MenuItemView;var b=SC.MenuPane.create({classNames:["select-button"],items:n,exampleView:j,isEnabled:YES,menuHeightPadding:e,preferType:SC.PICKER_MENU,itemHeightKey:"height",layout:{width:i},controlSize:q,itemWidth:i,contentView:SC.View.extend({})});
if(!b){return NO}b.popup(this,this.preferMatrix);b.set("currentSelectedMenuItem",p);
...
}else{l=""}e.push('<span class="sc-button-inner"><label class="sc-button-label">');
