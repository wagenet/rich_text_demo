...
}if(b.x<7){b.x=7}if(b.height>a.height){b.y=15;b.height=a.height-35}return b},fitPositionToScreenPointer:function(p,k,o){var n=(o.height>12)?0:1;
var l=(o.height>12)?0:3;var e=[[o.x+o.width+(7+n),o.y+parseInt(o.height/2,0)-40],[o.x-k.width-(7+n),o.y+parseInt(o.height/2,0)-40],[o.x+parseInt((o.width/2)-(k.width/2),0),o.y-k.height-(17+l)],[o.x+parseInt((o.width/2)-(k.width/2),0),o.y+o.height+(17+l)]];
var d=[[o.x+o.width+k.width+(7+n),o.y+parseInt(o.height/2,0)+k.height-24],[o.x-(7+n),o.y+parseInt(o.height/2,0)+k.height-24],[o.x+parseInt((o.width/2)-(k.width/2),0)+k.width,o.y-(17+l)],[o.x+parseInt((o.width/2)-(k.width/2),0)+k.width,o.y+o.height+k.height+(17+l)]];
var g=[[e[0][1]>0?0:0-e[0][1],d[0][0]<p.width?0:d[0][0]-p.width,d[0][1]<p.height?0:d[0][1]-p.height,e[0][0]>0?0:0-e[0][0]],[e[1][1]>0?0:0-e[1][1],d[1][0]<p.width?0:d[1][0]-p.width,d[1][1]<p.height?0:d[1][1]-p.height,e[1][0]>0?0:0-e[1][0]],[e[2][1]>0?0:0-e[2][1],d[2][0]<p.width?0:d[2][0]-p.width,d[2][1]<p.height?0:d[2][1]-p.height,e[2][0]>0?0:0-e[2][0]],[e[3][1]>0?0:0-e[3][1],d[3][0]<p.width?0:d[3][0]-p.width,d[3][1]<p.height?0:d[3][1]-p.height,e[3][0]>0?0:0-e[3][0]]];
if(!this.preferMatrix||this.preferMatrix.length!==5){this.set("preferMatrix",[0,1,2,3,2])
}var c=this.preferMatrix;if(c[4]===-1){k.x=o.x+parseInt(o.width/2,0);k.y=o.y+parseInt(o.height/2,0)-parseInt(k.height/2,0);
this.set("pointerPos",SC.POINTER_LAYOUT[0]+" fallback");this.set("pointerPosY",parseInt(k.height/2,0)-40)
}else{k.x=e[c[4]][0];k.y=e[c[4]][1];this.set("pointerPos",SC.POINTER_LAYOUT[c[4]]);
this.set("pointerPosY",0)}this.set("pointerPosX",0);for(var h=0,b,j=SC.POINTER_LAYOUT.length;
h<j;h++){b=c[h];if(g[b][0]===0&&g[b][1]===0&&g[b][2]===0&&g[b][3]===0){if(c[4]!==b){k.x=e[b][0];
k.y=e[b][1];this.set("pointerPosY",0);this.set("pointerPos",SC.POINTER_LAYOUT[b])
}h=SC.POINTER_LAYOUT.length}else{if((b===0||b===1)&&g[b][0]===0&&g[b][1]===0&&g[b][2]<k.height-91&&g[b][3]===0){if(c[4]!==b){k.x=e[b][0];
this.set("pointerPos",SC.POINTER_LAYOUT[b])}k.y=e[b][1]-g[b][2];this.set("pointerPosY",g[b][2]);
h=SC.POINTER_LAYOUT.length}else{if((b===0||b===1)&&g[b][0]===0&&g[b][1]===0&&g[b][2]<=k.height-57&&g[b][3]===0){if(c[4]!==b){k.x=e[b][0]
}k.y=e[b][1]-(k.height-57);this.set("pointerPosY",(k.height-59));this.set("pointerPos",SC.POINTER_LAYOUT[b]+" extra-low");
h=SC.POINTER_LAYOUT.length}}}}return k},displayProperties:["pointerPosY"],render:function(b,d){var a=arguments.callee.base.apply(this,arguments);
...
})}return b},render:function(b,e){arguments.callee.base.apply(this,arguments);var c,a,l,o,r,f,q,g,j,m,i,d,h,s,n,k,p;
c=this.layout.width;if(e&&c){this.adjust({width:c-this.SELECT_BUTTON_SPRITE_WIDTH})
}a=this.get("objects");a=this.sortObjects(a);l=a.length;o=this.get("nameKey");r=this.get("iconKey");
f=this.get("valueKey");q=this.get("checkboxEnabled");g=this.get("value");j=this.get("localize");
m=this.get("separatorPostion");i=[];d=YES;h=0;a.forEach(function(t){if(t){s=o?(t.get?t.get(o):t[o]):t.toString();
s=j?s.loc():s;n=r?(t.get?t.get(r):t[r]):null;if(SC.none(t[r])){n=null}k=(f)?(t.get?t.get(f):t[f]):t;
if(!SC.none(g)&&!SC.none(k)){if(g===k){this.set("title",s);this.set("icon",n)}}if(k===this.get("value")){this.set("itemIdx",h);
d=!q?NO:YES}else{d=NO}if(h===0){this._defaultVal=k;this._defaultTitle=s;this._defaultIcon=n
}var u=SC.Object.create({title:s,icon:n,value:k,isEnabled:YES,checkbox:d,action:this.displaySelectedItem});
i.push(u)}h+=1;if(m&&h===(l-m)){var v=SC.Object.create({separator:YES});i.push(v)
}this.set("itemList",i)},this);if(e){this.invokeLast(function(){var t=this.get("value");
if(SC.none(t)){this.set("value",this._defaultVal);this.set("title",this._defaultTitle);
...
if(i&&q){a=a+i-q}}if(!j||(a>j)){j=a}n=this.get("itemList");var r=this.get("customViewClassName");
...
d=r?(d+" "+r):d;for(m=0,v=n.length;m<v;++m){w=n.objectAt(m);c=document.createElement("div");
...
if(j>t){j=(t-25)}this.set("lastMenuWidth",j);s=this.get("currentSelItem");o=this.get("itemList");
u=this.get("controlSize");f=this.get("menuPaneHeightPadding");g=this.get("customView");
...
return a&&(a.get?a.get("isEditable")!==NO:NO)}.property("content").cacheable(),render:function(c,a){var f=this.get("content"),k=this.displayDelegate,b=this.get("outlineLevel"),e=this.get("outlineIndent"),j,i,h;
...
}this._invalidIndexes=NO;var g=this.get("content"),f=g?g.get("length"):0,e=this.computeLayout(),b=SC.BENCHMARK_RELOAD,a=this.get("nowShowing"),c=this._sc_itemViews,j=this.get("containerView")||this,l,k,i,d;
...
}h.forEach(function(m){var n=c?c[m]:null;if(a.contains(m)){i=this.itemViewForContentIndex(m,YES);
if(n&&n.parentView===j){d=n.get("layer");if(d&&d.parentNode){d.parentNode.removeChild(d)
}d=null;j.replaceChild(i,n)}else{j.appendChild(i)}}else{if(n&&n.parentView===j){delete c[m];
d=n.get("layer");if(d&&d.parentNode){d.parentNode.removeChild(d)}d=null;j.removeChild(n)
}}},this);if(b){SC.Benchmark.end(b)}}else{if(b){SC.Benchmark.start(b="%@#reloadIfNeeded (Full)".fmt(this),YES)
}if(c){c.length=0}l=[];a.forEach(function(m){l.push(this.itemViewForContentIndex(m,YES))
},this);j.beginPropertyChanges();j.destroyLayer().removeAllChildren();j.set("childViews",l);
...
}}else{if(l>0.45){b++}}var h=(m*k)+b;return[h,i]},_gv_clippingFrameDidChange:function(){var d=this.get("nowShowing"),c,b,a;
...
var d=k<h;var c=e<a;if(d||c){this.forceDimensionsRecalculation(c,d,h,a)}},_scroll_horizontalScrollOffsetDidChange:function(){var b=this.get("horizontalScrollOffset");
...
this.invokeLater(this._animateProgressBar,a,10)}},displayProperties:"value minimum maximum isIndeterminate".w(),render:function(c,b){var p,f,m,d,i,g=this.get("isIndeterminate"),o=this.get("isRunning"),l=this.get("isEnabled"),n=this.get("offsetRange"),h=(g&&o)?(Math.floor(Date.now()/75)%n-n):0;
if(!l){m="0%"}else{if(g){m="120%"}else{var j=this.get("minimum")||0;var e=this.get("maximum")||1;
m=this.get("value")||0;m=(m-j)/(e-j);if(m>1){m=1}if(isNaN(m)){m=0}if(m<j){m=0}if(m>e){m=1
}m=(m*100)+"%"}}var a={"sc-indeterminate":g,"sc-empty":(m<=0),"sc-complete":(m>=100)};
if(b){var k=this._createClassNameString(a);c.push('<div class="sc-inner ',k,'" style="width: ',m,";left: ",h,'">');
...
d="width: "+m+"; ";d=d+"left: "+h+"; ";if(f.length===3){i="0px -"+(f[0]+f[1]*this._currentBackground)+"px";
...
p.attr("style",d)}else{p.attr("style",d)}}},contentPropertyDidChange:function(c,a){var b=this.get("content");
...
},$input:function(){return this.$("input")},displayProperties:["value","displayItems"],render:function(e,a){var o,n,k,c,p,d,j,g,f,l,b,i=this.get("displayItems"),m=this.get("value"),h=SC.isArray(m);
e.addClass(this.get("layoutDirection"));if(h&&m.length<=0){m=m[0];h=NO}if(a){c=SC.guidFor(this);
p=i.length;for(n=0;n<p;n++){o=i[n];k=o[3];if(k){d=(k.indexOf("/")>=0)?k:SC.BLANK_IMAGE_URL;
j=(d===k)?"":k;k='<img src="%@" class="icon %@" alt="" />'.fmt(d,j)}else{k=""}b=this._getSelectionState(o,m,h,false);
g=(!o[2])||(!this.get("isEnabled"))?'disabled="disabled" ':"";f=this.escapeHTML?SC.RenderContext.escapeHTML(o[0]):o[0];
e.push('<label class="sc-radio-button ',b,'">');e.push('<input type="radio" value="',n,'" name="',c,'" ',g,"/>");
e.push('<span class="button"></span>');e.push('<span class="sc-button-label">',k,f,"</span></label>")
...
l=this._getSelectionState(o,m,h,true);q.parent().setClass(l);q=n=l=null},this)}},_getSelectionState:function(d,f,a,b){var e,h,c;
...
}else{var k=this.get("activeIndex");var h=this.get("value");var c=SC.isArray(h);if(c&&h.get("length")===1){h=h.objectAt(0);
c=NO}var g={};var d=f.length,e=this.$("a.sc-segment"),j;while(--d>=0){j=f[d];g.sel=c?(h.indexOf(j[1])>=0):(j[1]===h);
g.active=(k===d);g.disabled=!j[2];SC.$(e[d]).setClass(g)}g=f=h=f=null}},renderDisplayItems:function(d,k){var n=this.get("value"),g=SC.isArray(n),p=this.get("activeIndex"),h=k.length,m,l,b,j,e,o,a,c,f;
...
}if(l){b=(l.indexOf("/")>=0)?l:SC.BLANK_IMAGE_URL;j=(b===l)?"":l;l='<img src="'+b+'" alt="" class="icon '+j+'" />'
}else{l=""}e.push('<span class="sc-button-inner"><label class="sc-button-label">',l+m,"</label></span>");
